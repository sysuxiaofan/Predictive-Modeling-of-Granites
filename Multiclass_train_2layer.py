...
from sklearn.model_selection import GridSearchCV
from keras.wrappers.scikit_learn import KerasClassifier
def build_model(n_hidden=1, n_neurons1=30,n_neurons2=50, input_shape=[44]):
...
param_distribs = {
     ...
     "n_neurons2": [10,20,30,40,50,60,70,80,90]}
...
# c.to_csv('C:/Users/WKQ/Desktop/tangao代码/结果2层/acc.csv')
#Feature importance ranking
#model = build_model(n_neurons1=70,n_neurons2=80,input_shape=[44])
#X_train2,X_vaild,Y_train2,Y_vaild  = train_test_split(X_train,Y_train,test_size=0.2, random_state=2)
#Hit=model.fit(X_train2,Y_train2,validation_data=(X_vaild,Y_vaild),epochs=10,verbose=0)
# explainer = shap.DeepExplainer(model,tf.cast(tf.convert_to_tensor(X_vaild.values),tf.float32))
# shapvalue=explainer.shap_values(X_vaild)
# shap.summary_plot(shapvalue[1],X_vaild,plot_type="bar")
...
#     model2=build_model(n_neurons1=50,n_neurons2=40,input_shape=[43])
...
# unkown.to_csv('./Multiclass_predict.csv',index=False)
